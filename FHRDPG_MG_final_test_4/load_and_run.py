...
def load_and_print(args=None):
    ...
    for i in range(num_episode):
        ...
        while time < t_max:
            ...
            # path_actor = '../models_and_tb/FHRDPG-7days/seed'+str(env.seed)+'/models/DDPG' + '_TS_' + str(time + 1) + '_LR_' + str(algo.lr) + '_actor.h5'
            # path_critic = '../models_and_tb/FHRDPG-7days/seed' + str(env.seed) + '/models/DDPG' + '_TS_' + str(time + 1) + '_LR_' + str(algo.lr) + '_critic.h5'
            model_seed = 1
            path_actor = '../models_and_tb/FHRDPG-21days/seed' + str(model_seed) + '/models/DDPG' + '_TS_' + str(time + 1) + '_LR_' + str(algo.lr) + '_actor.h5'
            path_critic = '../models_and_tb/FHRDPG-21days/seed' + str(model_seed) + '/models/DDPG' + '_TS_' + str(time + 1) + '_LR_' + str(algo.lr) + '_critic.h5'

            algo.actor.load_weights(path_actor)
            algo.critic.load_weights(path_critic)
            # state_norm = np.expand_dims(state_norm, axis=0)
            history_norm[-1, -1] = state_norm[1]
            a_norm = algo.policy_action(history_norm)
...
#                env.cnt += 1
            ...
            # evaluate critic
            r_true.append(r[0])
            history_norm_for_q = history_norm.copy()
            history_norm_for_q = np.expand_dims(history_norm_for_q, axis=0)
            a_norm_for_q = a_norm.copy()
            a_norm_for_q = np.expand_dims(a_norm_for_q, axis=0)
            q_value = algo.critic.target_predict([history_norm_for_q, a_norm_for_q])
            q_estimated.append(q_value[0][0])

            history_t1_norm = np.reshape(history_norm, [(env.hisStep) * env.stateDim])
            history_t1_norm[0: -env.stateDim - 1] = history_t1_norm[env.stateDim:-1]
            history_t1_norm[-env.stateDim:] = state_norm[0:]
            history_t1_norm = np.reshape(history_t1_norm, [env.hisStep, env.stateDim])
            history_norm = history_t1_norm
    ...
    x = range(0, int(24/dt))
    plt.subplot(611)
    plt.plot(x, env.PPV[env.hisStep:-1], marker='.', mec='r', mfc='w')
    plt.xlabel("step")
    plt.ylabel("PV")

    plt.subplot(612)
    plt.plot(x, env.PL[env.hisStep:-1], marker='.', mec='r', mfc='w')
    plt.xlabel("step")
    plt.ylabel("Load")
