...
class env_mg:
    ...
    def step(self,u):
        ...
        # E_t = self.state[2] # E_t_1 is E_t-1，E_t1 is E_t+1
        E_t = self.state[1]
    ...
    def load_data(self, n_days):
        ...
        n = 517 + 14*24
        t_max = 24 * n_days
        self.PL = np.zeros(shape=[t_max])
        self.PPV = np.zeros(shape=[t_max])
    ...
    def reset(self,t):
        ...
        PL_t = float(self.PL[t])
        PPV_t = float(self.PPV[t])
        # self.cnt += 1
        self.cnt = t
        ...
        self.state = state
        # return state
        # obs = np.array([PL_t_1, PPV_t_1, self.E_t])
        # self.obs = obs
        return state
    ...
    def baseline_local_opt_run(self, num_episode):
        ...
        for i in range(num_episode):
            ...
            for j in range(t_max-1):
                ...
                self.cus.pop()
                self.cdg.pop()
                state, r, _d, _ = self.step(a)  #r should be derived by myopic_policy，we derive state here
                r *= self.rewardScale
                totalReward += r
                dg.append(a)
                print("a:", a)
