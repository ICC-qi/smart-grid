...
class Critic:
    ...
    def __init__(self, inp_dim, out_dim, lr, tau, act_range, act_bias, seed):
...
#        xa = Lambda(lambda i: (i - self.act_bias) / self.act_range)(action)
        # x = Dense(256, activation='relu', kernel_initializer=RandomUniform(minval = -1/np.sqrt(256), maxval = 1/np.sqrt(256), seed = self.seed))(state)
        x = Dense(400, activation='relu', kernel_initializer=RandomUniform(minval = -1/np.sqrt(400), maxval = 1/np.sqrt(400), seed = self.seed))(state)
